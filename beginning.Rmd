...
```{r, echo = FALSE, warning=FALSE}
ggplot (data=cia, aes(x=birth_rate, y=life_exp_at_birth)) + geom_point(aes(color = continent,size=population))
```


```{r,echo=FALSE,warning=FALSE}
ggplot (data=cia, aes(x=death_rate)) + geom_histogram(bins = 30, aes(fill = continent))
```
```{r,echo=FALSE,warning=FALSE}
ggplot (data=cia, aes(x=life_exp_at_birth)) + geom_histogram(bins = 30, aes(color = death_rate,fill=continent))
```
